    ...
    def get_page_content(url: str, header: dict = None, retry_time: int = 3) -> tuple:
        ...
        while fail_count < retry_time:
            try:
                ...
                result = re.sub(r'<style.*?>.*?/style>', "###style texts###", result, flags=re.S)
                # 削去所有 js 内容（可能有问题）
                result = re.sub(r'<script.*?>.*?/script>', "###java script###", result, flags=re.S)
                # 返回页面内容和真实响应url
...
if __name__ == "__main__":
    a = HTMLDownloader.get_page_content("https://baike.baidu.com/item/%E5%9B%BD%E5%AE%B6%E4%BD%93%E8%82%B2%E5%9C%BA")
    print(a)
