...
class MysqlConnector:
    ...
    #         if not strict_mode or (strict_mode and len(columns) == len(values)):
    #             max_num = max(0, min(len(columns), len(values)) - 1)  # 取最小,多余的列或者值被丢弃
    #             sqlstmt += "("
    #             value_statement = "("
    #             for i in range(0, max_num):
    #                 sqlstmt += self.input_verify(columns[i]) + ","
    #                 if values[i] is str:
    #                     value_statement += "'" + self.input_verify(values[i]) + "',"
    #                 else:
    #                     value_statement += "" + self.input_verify(values[i]) + ","
    #             sqlstmt = sqlstmt[:-1] + ") VALUES" + value_statement[:-1] + ")"
    #         else:
    #             print("Arg nums not match.Insert fail")
    #             return False
    #     else:
    #         sqlstmt += " VALUES ("
    #         for value in values:
    #             if type(value) is str:
    #                 sqlstmt += "'" + self.input_verify(value) + "',"
    #             else:
    #                 sqlstmt += "" + str(self.input_verify(value)) + ","
    #         sqlstmt = sqlstmt[:-1] + ")"
    #     self.execute_raw_sql(sqlstmt)
